...
void  DLL_EXPORT mod(int a[MAX],int b[MAX],int  *c)/*/c=a mod b//注意：经检验知道此处A和C的数组都改变了。*/
{
    int d[MAX];
    mov (a,d);
...
void  DLL_EXPORT divt(int t[MAX],int b[MAX],int  *c,int *w) /*//试商法//调用以后w为a mod b, C为a  div b;*/
{
...
void DLL_EXPORT mulmod(int a[MAX],int b[MAX],int n[MAX],int *m)  /*解决 了 m=a*b mod n;*/
{
    int c[MAX],d[MAX];
    int i;
    for(i=0; i<MAX; i++)
...
int DLL_EXPORT coprime(int e[MAX],int s[MAX]) /*//// 求两个大数之间是否互质////*/
...
unsigned long  DLL_EXPORT rsa(unsigned long p,unsigned long q,unsigned long e)  /*/求解密密钥d的函数(根据Euclid算法)*/
{
...
/*/------------------------------------------导入导出公钥和私钥------------------------------------/*/
void DLL_EXPORT loadpkey(int e[MAX],int n[MAX])  //导入公钥
{
    FILE *fp;
...
void DLL_EXPORT loadskey(int d[MAX],int n[MAX]) //导入私钥
{
    {
        FILE *fp;
        char filename[25],str[MAX],ch;
...
void DLL_EXPORT savepkey(int e[MAX],int n[MAX])//导出公钥
{
    FILE *fp;
    int  i;
    char  savefile[25],ch;
...
void DLL_EXPORT saveskey(int d[MAX],int n[MAX])//导出私钥
{
    FILE *fp;
    int  i;
    char  savefile[25],ch;
...
void  DLL_EXPORT tencrypto(int e[MAX], int n[MAX])/*//对有需要的文件进行加密*/
{
    FILE *fp;
...
struct DLL_EXPORT slink *input(void)/*/输入明文并且返回头指针，没有加密时候转化的数字*/
{
    struct  slink *head;
    struct  slink *p1,*p2;
    int  i,n,c,temp;
...
struct DLL_EXPORT slink *jiami(int  e[MAX],int  n[MAX],struct  slink *head)
{
    struct  slink *p;
    struct  slink *h;
    struct  slink *p1,*p2;
